...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addMusic () {
        var sound = new Howl({
            src: ['../music/Progulka_v_lesu_den_oum_ru.mp3'],
            loop: true,
            autoplay: false
        });
        ...
        sound.once('load', function(){
            sound.play();
            soundBtn.classList.remove('dnone');
        });
        soundBtn.onclick = function() {
            if (check) {
                this.innerHTML = '<img src="img/icons/1.svg" alt="resume"></img>';
                sound.mute(true);
                check = false;
            } else {
                this.innerHTML = '<img src="img/icons/2.svg" alt="mute"></img>';
                sound.mute(false);
                check = true;
            }
        };
    ...
    if (confirm('Хотите ли вы включить звуки леса?')) {
        addMusic();
    }
});
